...
Edit `README.md` for usage and setup, `info.plist` for the workflow graph, and `scripts/lists.js` for the local list reader. The source Script Filter `script` field is a placeholder; the build replaces it with `lists.js` (and embeds the Readme) without changing the source. Always import the built `.alfredworkflow` file.
